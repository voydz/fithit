...
def _search(
    category: str | None = typer.Option(
        None, "--category", help="Workout-Kategorie (Yoga, Strength, Core, ...)."
    ),
    categories: str | None = typer.Option(
        None, "--categories", help="Komma-separierte Kategorien."
    ),
    duration: str | None = typer.Option(
        None, "--duration", help="Exakte Dauer, z.B. '20 min'."
    ),
    max_duration: int | None = typer.Option(
        None, "--max-duration", help="Maximale Dauer in Minuten."
    ),
    equipment_free: bool = typer.Option(
        False, "--equipment-free", help="Nur bodyweight/mat Workouts."
    ),
    trainer: str | None = typer.Option(None, "--trainer", help="Trainer-Name."),
    body_focus: str | None = typer.Option(
        None, "--body-focus", help="Body Focus (Upper/Lower/Total Body)."
    ),
    flow_style: str | None = typer.Option(
        None, "--flow-style", help="Yoga Flow Style (Slow, Energetic, ...)."
    ),
    search: str | None = typer.Option(
        None, "--search", help="Textsuche in Beschreibung/Name."
    ),
    ...
    format: str = typer.Option(
        "compact", "--format", help="Ausgabeformat: compact|json."
    ),
):
...
@app.command("parse")
def _parse(
    dtable_path: str = typer.Argument(
        ..., help="Pfad zur .dtable Datei (ZIP mit content.json)."
    ),
    output: str | None = typer.Option(
        None, "--output", help="Zielpfad für workouts.json (default: Standard-DB-Pfad)."
    ),
):
